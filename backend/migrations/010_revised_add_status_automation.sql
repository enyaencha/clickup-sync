-- Migration: Add Status Automation (Compatible with Dec 9, 2025 Schema)
-- Date: 2025-12-09
-- Description: Adds status automation fields compatible with existing schema

-- ================================================
-- ACTIVITIES: Add Automation Fields (progress_percentage already exists as INT)
-- ================================================
...
ADD COLUMN IF NOT EXISTS manual_status VARCHAR(50) COMMENT 'Manually set status',
ADD COLUMN IF NOT EXISTS status_reason TEXT COMMENT 'Explanation for status',
ADD COLUMN IF NOT EXISTS last_status_update TIMESTAMP NULL COMMENT 'Last status update',
ADD COLUMN IF NOT EXISTS risk_level ENUM('none', 'low', 'medium', 'high', 'critical') DEFAULT 'none';

-- Add indices
ALTER TABLE activities
ADD INDEX IF NOT EXISTS idx_auto_status (auto_status),
...
-- Rename status to overall_status for consistency, OR add new column
-- Option 1: Add new overall_status column (safer, keeps existing data)
ALTER TABLE project_components
ADD COLUMN IF NOT EXISTS overall_status VARCHAR(50) COMMENT 'Rolled-up status from activities',
ADD COLUMN IF NOT EXISTS status_override BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS auto_status VARCHAR(50),
ADD COLUMN IF NOT EXISTS manual_status VARCHAR(50),
...
ALTER TABLE sub_programs
ADD COLUMN IF NOT EXISTS overall_status VARCHAR(50) COMMENT 'Rolled-up status',
ADD COLUMN IF NOT EXISTS status_override BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS auto_status VARCHAR(50),
ADD COLUMN IF NOT EXISTS manual_status VARCHAR(50),
ADD COLUMN IF NOT EXISTS last_status_update TIMESTAMP NULL,
...
ADD COLUMN IF NOT EXISTS overall_status VARCHAR(50) DEFAULT 'not-started',
ADD COLUMN IF NOT EXISTS status_override BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS last_status_update TIMESTAMP NULL;

-- ================================================
-- ME_INDICATORS: Add Missing Fields Only
-- (baseline_value, target_value, current_value, achievement_percentage already exist!)
-- ================================================

-- Add only the missing fields
ALTER TABLE me_indicators
ADD COLUMN IF NOT EXISTS variance DECIMAL(15,2) DEFAULT 0.00 COMMENT 'current_value - target_value',
ADD COLUMN IF NOT EXISTS performance_status ENUM(
    'not-started','on-track','at-risk','off-track','achieved','exceeded'
) DEFAULT 'not-started' COMMENT 'Performance status';

-- Update status enum if it exists (me_indicators.status already has: on-track, at-risk, off-track, not-started)
-- This matches our needs, so no change needed for status column

-- Add index
ALTER TABLE me_indicators
ADD INDEX IF NOT EXISTS idx_performance_status (performance_status),
ADD INDEX IF NOT EXISTS idx_variance (variance);
...
SET variance = CASE
    ...
    ELSE 0
END
WHERE variance IS NULL OR variance = 0;

-- ================================================
-- NEW TABLES (Same as before)
-- ================================================
...
SELECT
    'Migration 010-revised completed successfully!' as message,
