...
// ============================
    ...
    async pullUsers() {
        ...
        // First, get all teams (since users are scoped under teams in ClickUp API)
        const teams = await this.db.query('SELECT clickup_id FROM teams');

        for (const team of teams) {
            try {
                const clickupUsers = await this.clickupAPI.getUsers(team.clickup_id);

                if (!clickupUsers || !clickupUsers.members) continue;

                for (const member of clickupUsers.members) {
                    await this.upsertUser(member.user);
                }
            } catch (err) {
                console.error(`Failed to pull users for team ${team.clickup_id}:`, err.message);
            }
        }
    }
    ...
    async upsertUser(clickupUser) {
        ...
        const userData = {
            ...
            initial: clickupUser.initials,
            clickup_updated_at: clickupUser.date_updated
                ? new Date(parseInt(clickupUser.date_updated))
                : new Date()
        };
        ...
        if (existingUser.length === 0) {
            ...
            await this.db.query(`
            INSERT INTO users (
                clickup_id, username, email, color, profile_picture,
                role, initial, clickup_created_at, clickup_updated_at,
                sync_status, last_sync_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'synced', NOW())
        `, [
                clickupUser.id,
                ...
                userData.role,
                userData.initial,
                clickupUser.date_created
                    ? new Date(parseInt(clickupUser.date_created))
                    : new Date(),
                userData.clickup_updated_at
            ]);
            ...
            if (userData.clickup_updated_at > existingUpdated) {
                await this.db.query(`
                UPDATE users 
                SET username = ?, email = ?, color = ?, profile_picture = ?, 
                    role = ?, initial = ?, clickup_updated_at = ?, 
...
// ============================
    async pullFolders() {
        ...
        const spaces = await this.db.query('SELECT clickup_id FROM spaces');

        for (const space of spaces) {
            try {
                const folders = await this.clickupAPI.getFolders(space.clickup_id);

                if (!folders || !folders.folders) continue;

                for (const folder of folders.folders) {
                    await this.upsertFolder(folder, space.clickup_id);
                }
            } catch (err) {
                console.error(`Failed to pull folders for space ${space.clickup_id}:`, err.message);
            }
        }
    }
    ...
    async upsertFolder(clickupFolder, spaceId) {
        ...
        const folderData = {
            ...
            hidden: clickupFolder.hidden ? 1 : 0,
            override_statuses: clickupFolder.override_statuses ? 1 : 0,
            clickup_updated_at: clickupFolder.date_updated
                ? new Date(parseInt(clickupFolder.date_updated))
                : new Date()
        };
        ...
        if (existing.length === 0) {
            await this.db.query(`
            INSERT INTO folders (
                clickup_id, space_id, name, hidden, override_statuses,
        ...
        `, [
                ...
                folderData.name,
                folderData.hidden,
                folderData.override_statuses,
                clickupFolder.date_created
                    ? new Date(parseInt(clickupFolder.date_created))
                    : new Date(),
                folderData.clickup_updated_at
            ]);

            console.log(`Created folder: ${folderData.name}`);
        } else {
            const existingUpdated = new Date(existing[0].clickup_updated_at);
            if (folderData.clickup_updated_at > existingUpdated) {
                await this.db.query(`
                UPDATE folders
                SET name = ?, hidden = ?, override_statuses = ?, 
...
// ============================
    async pullLists() {
        ...
        const folders = await this.db.query('SELECT clickup_id FROM folders');

        for (const folder of folders) {
            try {
                const lists = await this.clickupAPI.getLists(folder.clickup_id);

                if (!lists || !lists.lists) continue;

                for (const list of lists.lists) {
                    await this.upsertList(list, folder.clickup_id);
                }
            } catch (err) {
                console.error(`Failed to pull lists for folder ${folder.clickup_id}:`, err.message);
            }
        }
    }

    async upsertList(clickupList, folderId) {
        const existing = await this.db.query(
        ...
        const listData = {
            ...
            archived: clickupList.archived ? 1 : 0,
            clickup_updated_at: clickupList.date_updated
                ? new Date(parseInt(clickupList.date_updated))
                : new Date()
        };
        ...
        if (existing.length === 0) {
            await this.db.query(`
            INSERT INTO lists (
                clickup_id, folder_id, name, archived,
                clickup_created_at, clickup_updated_at, sync_status, last_sync_at
            ) VALUES (?, ?, ?, ?, ?, ?, 'synced', NOW())
        `, [
                clickupList.id,
                ...
                listData.name,
                listData.archived,
                clickupList.date_created
                    ? new Date(parseInt(clickupList.date_created))
                    : new Date(),
            ...
            const existingUpdated = new Date(existing[0].clickup_updated_at);
            if (listData.clickup_updated_at > existingUpdated) {
                await this.db.query(`
                UPDATE lists 
                SET name = ?, archived = ?, clickup_updated_at = ?, 
                    sync_status = 'synced', last_sync_at = NOW()
                WHERE clickup_id = ?
            `, [
                    listData.name,
...
// ============================
    async pullComments() {
        ...
        const tasks = await this.db.query('SELECT clickup_id FROM tasks');

        for (const task of tasks) {
            try {
                const comments = await this.clickupAPI.getComments(task.clickup_id);

                if (!comments || !comments.comments) continue;

                for (const comment of comments.comments) {
                    await this.upsertComment(comment, task.clickup_id);
                }
            } catch (err) {
                console.error(`Failed to pull comments for task ${task.clickup_id}:`, err.message);
            }
        }
    }
    ...
    async upsertComment(clickupComment, taskId) {
        ...
        const commentData = {
            ...
            user_id: clickupComment.user?.id || null,
            clickup_updated_at: clickupComment.date_updated
                ? new Date(parseInt(clickupComment.date_updated))
                : new Date()
        };
        ...
        `, [
                ...
                clickupComment.date_created
                    ? new Date(parseInt(clickupComment.date_created))
                    : new Date(),
                commentData.clickup_updated_at
            ]);
            console.log(`Created comment: ${commentData.comment_text?.substring(0, 20)}...`);
        } else {
            const existingUpdated = new Date(existing[0].clickup_updated_at);
            if (commentData.clickup_updated_at > existingUpdated) {
                await this.db.query(`
                UPDATE comments
                SET user_id = ?, comment_text = ?, clickup_updated_at = ?, 
                    sync_status = 'synced', last_sync_at = NOW()
                WHERE clickup_id = ?
            `, [
                    commentData.user_id,
                ...
                console.log(`Updated comment: ${commentData.comment_text?.substring(0, 20)}...`);
            }
        }
    }
...
// ============================
    async pullTimeEntries() {
        ...
        for (const task of tasks) {
            try {
                const entries = await this.clickupAPI.getTimeEntries(task.clickup_id);

                if (!entries || !entries.data) continue;

                for (const entry of entries.data) {
                    await this.upsertTimeEntry(entry, task.clickup_id);
                }
            } catch (err) {
                console.error(`Failed to pull time entries for task ${task.clickup_id}:`, err.message);
            }
        }
    }

    async upsertTimeEntry(clickupEntry, taskId) {
        const existing = await this.db.query(
        ...
        const entryData = {
            user_id: clickupEntry.user?.id || null,
            duration: clickupEntry.duration || 0,
            description: clickupEntry.description || null,
            clickup_updated_at: clickupEntry.date_updated
                ? new Date(parseInt(clickupEntry.date_updated))
                : new Date()
        };
        ...
        if (existing.length === 0) {
            ...
            INSERT INTO time_entries (
                clickup_id, task_id, user_id, duration, description,
                start, end, clickup_created_at, clickup_updated_at, sync_status, last_sync_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'synced', NOW())
        `, [
                clickupEntry.id,
                taskId,
                ...
                clickupEntry.start ? new Date(parseInt(clickupEntry.start)) : null,
                clickupEntry.end ? new Date(parseInt(clickupEntry.end)) : null,
                clickupEntry.date_created
                    ? new Date(parseInt(clickupEntry.date_created))
                    : new Date(),
                entryData.clickup_updated_at
            ]);
            console.log(`Created time entry for task ${taskId}`);
        } else {
            const existingUpdated = new Date(existing[0].clickup_updated_at);
            if (entryData.clickup_updated_at > existingUpdated) {
                await this.db.query(`
                UPDATE time_entries
                SET user_id = ?, duration = ?, description = ?, start = ?, end = ?, 
                    clickup_updated_at = ?, sync_status = 'synced', last_sync_at = NOW()
                WHERE clickup_id = ?
            `, [
                    entryData.user_id,
                ...
                console.log(`Updated time entry for task ${taskId}`);
            }
        }
    }
