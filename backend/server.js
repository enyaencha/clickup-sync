...
//const PullStatuses = require('./services/pullStatuses');
const PullCustomFields = require('./services/ pullCustomFields');
const PullTaskCustomFields = require('./services/pullTaskCustomFields');
const PullTags = require('./services/pullTags');
const PullChecklists = require('./services/pullChecklists');
const PullAttachments = require('./services/pullAttachments');
const PullGoals = require('./services/pullGoals');
const PullViews = require('./services/pullViews');
const pullTaskTags = require('./services/pullTaskTags')
...
class ClickUpAPI {
    ...
    // Add these methods to your ClickUpAPI class after the makeRequest method
    async get(endpoint) {
...
class SyncEngine {
    constructor(db, clickupAPI) {
        ...
        // inside SyncEngine constructor
       // this.pullStatuses = new PullStatuses(db, clickupAPI);
        this.pullCustomFields = new PullCustomFields(db, clickupAPI);
        this.pullTaskCustomFields = new PullTaskCustomFields(db, clickupAPI);
        this.pullTags = new PullTags(db, clickupAPI);
        this.pullChecklists = new PullChecklists(db, clickupAPI);
        this.pullAttachments = new PullAttachments(db, clickupAPI);
        this.pullGoals = new PullGoals(db, clickupAPI);
        this.pullViews = new PullViews(db, clickupAPI);
        this.pullTaskTags = new pullTaskTags(db, clickupAPI);
...
// ============================
    ...
    async pullAllData() {
        ...
        try {
            // 1. Teams
            await this.pullTeams();
            const teams = await this.db.query('SELECT * FROM teams WHERE is_active = 1');

            // 2. Users (must exist before referencing them)
            await this.pullUsers();

            // 3. Spaces (depends on teams)
            await this.pullSpaces();
            const spaces = await this.db.query('SELECT * FROM spaces WHERE is_active = 1');

            // 4. Folders (depends on spaces)
            await this.pullFolders();

            // 5. Lists (depends on spaces & folders)
            await this.pullLists();
            const lists = await this.db.query('SELECT * FROM lists WHERE is_active = 1');

            // 6. Statuses (depends on lists) - DISABLED for now
            // for (const list of lists) {
            //     await this.pullStatuses.pull(list);
            // }

            // 7. Custom fields (depends on lists)
            for (const list of lists) {
                await this.pullCustomFields.pull(list.id, list.clickup_id);
            }

            // 8. Views (depends on lists)
            for (const list of lists) {
                await this.pullViews.pull(list.id, list.clickup_id);
            }

            // 9. Tags (depends on spaces)
            for (const space of spaces) {
                await this.pullTags.pull(space.id, space.clickup_id);
            }

            // 10. Tasks (depends on lists, users)
            await this.pullTasks();
            const tasks = await this.db.query('SELECT * FROM tasks WHERE is_active = 1');

            // Pull task tags
            for (const task of tasks) {
                await this.pullTaskTags.pull(task.id, task.clickup_id);
            }
            //task.space_id = undefined;


            // 11. Task custom fields (depends on tasks + custom fields)
            for (const task of tasks) {
                await this.pullTaskCustomFields.pull(task.id, task.clickup_id);
            }

            // 12. Checklists (depends on tasks)

            for (const task of tasks) {
                await this.pullChecklists.pull(task.id, task.clickup_id); // ✅ pass both
            }


            // 13. Attachments (depends on tasks)
            for (const task of tasks) {
                await this.pullAttachments.pull(task.id, task.clickup_id);
            }

            // 14. Goals (depends on teams)
            for (const team of teams) {
                await this.pullGoals.pull(team.id, team.clickup_id);
            }

            // 15. Comments (depends on tasks, users)
            await this.pullComments();

            // 16. Time entries (depends on teams, tasks, users)
            await this.pullTimeEntries();

            console.log('\n========== FULL PULL COMPLETED ==========\n');
        } catch (error) {
            console.error('❌ Full pull failed:', error);
        }
    }
    ...
    // Add these methods to your SyncEngine class

// ============================
// MISSING PULL METHODS
// ============================
    ...
    async pullUsers() {
        ...
        try {
            const res = await this.clickupAPI.makeRequest('GET', `/team`);

            if (!res || !res.teams) {
                console.log('No teams returned from ClickUp');
                return;
            }
            ...
            console.log('Finished pulling users');
        } catch (err) {
            console.error('Failed to pull users:', err.message || err);
        }
    }
    ...
    async upsertUser(clickupUser) {
        if (!clickupUser || !clickupUser.id) return null;
        ...
        const userData = {
            ...
            color: clickupUser.color || '#000000',
            profile_picture: clickupUser.profilePicture || null,
        ...
        if (existingUser.length === 0) {
            ...
            INSERT INTO users (
                clickup_id, username, email, color, 
                profile_picture, initials, sync_status, last_sync_at
            ) VALUES (?, ?, ?, ?, ?, ?, 'synced', NOW())
        `, [
                clickupUser.id,
                userData.username,
            ...
            console.log(`Created user: ${userData.username || clickupUser.id}`);
            return result.insertId;
        } else {
            await this.db.query(`
            UPDATE users 
            SET username = ?, email = ?, color = ?, 
                profile_picture = ?, initials = ?, 
    ...
    async pullFolders() {
        ...
        const spaces = await this.db.query('SELECT * FROM spaces WHERE is_active = true');

        for (const space of spaces) {
            try {
                const clickupFolders = await this.clickupAPI.getFolders(space.clickup_id);

                for (const folder of clickupFolders.folders) {
                    await this.upsertFolder(folder, space.id);
                }
            } catch (error) {
                console.error(`Failed to pull folders for space ${space.name}:`, error);
            }
        }
    }

    async upsertFolder(clickupFolder, spaceId) {
        const existingFolder = await this.db.query(
            'SELECT * FROM folders WHERE clickup_id = ?',
            [clickupFolder.id]
        );

        const folderData = {
            name: clickupFolder.name,
            orderindex: clickupFolder.orderindex || 0,
            is_hidden: clickupFolder.hidden || false
        };

        if (existingFolder.length === 0) {
            await this.db.query(`
            INSERT INTO folders (
                clickup_id, space_id, name, orderindex, 
                is_hidden, sync_status, last_sync_at
            ) VALUES (?, ?, ?, ?, ?, 'synced', NOW())
        `, [
                clickupFolder.id,
                spaceId,
                folderData.name,
                folderData.orderindex,
                folderData.is_hidden
            ]);
    ...
    async pullLists() {
        console.log('Pulling lists...');

        // Pull folderless lists from spaces
        const spaces = await this.db.query('SELECT * FROM spaces WHERE is_active = true');
        for (const space of spaces) {
            try {
                const clickupLists = await this.clickupAPI.getLists(space.clickup_id, true);

                for (const list of clickupLists.lists) {
                    await this.upsertList(list, space.id, null);
                }
            } catch (error) {
                console.error(`Failed to pull lists for space ${space.name}:`, error);
            }
        }

        // Pull lists from folders
        const folders = await this.db.query('SELECT * FROM folders WHERE is_active = true');
        for (const folder of folders) {
            try {
                const clickupLists = await this.clickupAPI.getFolderLists(folder.clickup_id);

                for (const list of clickupLists.lists) {
                    await this.upsertList(list, folder.space_id, folder.id);
                }
            } catch (error) {
                console.error(`Failed to pull lists for folder ${folder.name}:`, error);
            }
        }
    }

    async upsertList(clickupList, spaceId, folderId = null) {
        const existingList = await this.db.query(
            'SELECT * FROM lists WHERE clickup_id = ?',
            [clickupList.id]
        );

        const listData = {
            name: clickupList.name,
            orderindex: clickupList.orderindex || 0,
            content: clickupList.content || '',
            status: clickupList.status?.status || 'active',
            priority: clickupList.priority?.priority || null,
            assignee_id: clickupList.assignee ? await this.upsertUser(clickupList.assignee) : null,
            due_date: clickupList.due_date ? new Date(parseInt(clickupList.due_date)) : null,
            due_date_time: clickupList.due_date_time || false,
            start_date: clickupList.start_date ? new Date(parseInt(clickupList.start_date)) : null,
            start_date_time: clickupList.start_date_time || false
        };

        if (existingList.length === 0) {
            await this.db.query(`
            INSERT INTO lists (
                clickup_id, space_id, folder_id, name, orderindex, 
                content, status, priority, assignee_id, due_date, 
                due_date_time, start_date, start_date_time,
                sync_status, last_sync_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'synced', NOW())
        `, [
                clickupList.id,
                ...
                listData.status,
                listData.priority,
                listData.assignee_id,
                listData.due_date,
                listData.due_date_time,
                listData.start_date,
                listData.start_date_time
            ]);
            ...
            await this.db.query(`
            UPDATE lists 
            SET name = ?, orderindex = ?, content = ?, status = ?, 
                priority = ?, assignee_id = ?, due_date = ?, due_date_time = ?,
                start_date = ?, start_date_time = ?, folder_id = ?,
                sync_status = 'synced', last_sync_at = NOW()
            WHERE clickup_id = ?
        `, [
                listData.name,
    ...
    async pullComments() {
        ...
        const tasks = await this.db.query('SELECT * FROM tasks WHERE is_active = true');

        for (const task of tasks) {
            try {
                const clickupComments = await this.clickupAPI.getComments(task.clickup_id);

                for (const comment of clickupComments.comments) {
                    await this.upsertComment(comment, task.id);
                }
            } catch (error) {
                console.error(`Failed to pull comments for task ${task.name}:`, error);
            }
        }
    }

    async upsertComment(clickupComment, taskId) {
        const existingComment = await this.db.query(
            'SELECT * FROM comments WHERE clickup_id = ?',
            [clickupComment.id]
        );

        // Ensure user exists
        const userId = await this.upsertUser(clickupComment.user);

        const commentData = {
            comment_text: clickupComment.comment_text || '',
            clickup_created_at: new Date(parseInt(clickupComment.date)),
            clickup_updated_at: new Date(parseInt(clickupComment.date))  // Comments don't have separate update time
        };

        if (existingComment.length === 0) {
            await this.db.query(`
            INSERT INTO comments (
                clickup_id, task_id, user_id, comment_text,
                clickup_created_at, clickup_updated_at, sync_status, last_sync_at
            ) VALUES (?, ?, ?, ?, ?, ?, 'synced', NOW())
        `, [
                clickupComment.id,
                taskId,
                ...
                commentData.clickup_created_at,
                commentData.clickup_updated_at
            ]);

            console.log(`Created comment for task ${taskId}`);
        } else {
            // Comments are typically immutable, but we'll update in case there are edits
            await this.db.query(`
            UPDATE comments 
            SET comment_text = ?, sync_status = 'synced', last_sync_at = NOW()
            WHERE clickup_id = ?
        `, [
                commentData.comment_text,
                clickupComment.id
    ...
    async pullTimeEntries() {
        console.log('Pulling time entries...');
        const teams = await this.db.query('SELECT * FROM teams WHERE is_active = true');

        for (const team of teams) {
            try {
                const clickupTimeEntries = await this.clickupAPI.getTimeEntries(team.clickup_id, {
                    start_date: this.getLastSyncDate(),
                    end_date: Date.now()
                });

                for (const timeEntry of clickupTimeEntries.data) {
                    await this.upsertTimeEntry(timeEntry, team.id);
                }
            } catch (error) {
                console.error(`Failed to pull time entries for team ${team.name}:`, error);
            }
        }
    }

    async upsertTimeEntry(clickupTimeEntry, teamId) {
        const existingTimeEntry = await this.db.query(
            'SELECT * FROM time_entries WHERE clickup_id = ?',
            [clickupTimeEntry.id]
        );

        // Ensure user exists
        const userId = await this.upsertUser(clickupTimeEntry.user);

        // Get task ID if the time entry is associated with a task
        let taskId = null;
        if (clickupTimeEntry.task?.id) {
            const task = await this.db.query(
                'SELECT id FROM tasks WHERE clickup_id = ?',
                [clickupTimeEntry.task.id]
            );
            taskId = task.length > 0 ? task[0].id : null;
        }

        const timeEntryData = {
            description: clickupTimeEntry.description || '',
            time: clickupTimeEntry.time || 0,
            source: clickupTimeEntry.source || 'clickup',
            clickup_created_at: new Date(parseInt(clickupTimeEntry.start)),
            clickup_updated_at: new Date(parseInt(clickupTimeEntry.end || clickupTimeEntry.start))
        };

        if (existingTimeEntry.length === 0) {
            await this.db.query(`
            INSERT INTO time_entries (
                clickup_id, team_id, task_id, user_id, description, 
                time, source, clickup_created_at, clickup_updated_at,
                sync_status, last_sync_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'synced', NOW())
        `, [
                clickupTimeEntry.id,
                ...
                timeEntryData.time,
                timeEntryData.source,
                timeEntryData.clickup_created_at,
                timeEntryData.clickup_updated_at
            ]);

            console.log(`Created time entry: ${timeEntryData.description}`);
        } else {
            await this.db.query(`
            UPDATE time_entries 
            SET description = ?, time = ?, source = ?, 
                clickup_updated_at = ?, sync_status = 'synced', last_sync_at = NOW()
            WHERE clickup_id = ?
        `, [
                timeEntryData.description,
                timeEntryData.time,
...
// ============================
    ...
    getLastSyncDate() {
        // Return timestamp from 30 days ago as default
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        return thirtyDaysAgo.getTime();
    }

    async handleTaskDeletion(webhookData) {
        console.log(`Handling task deletion: ${webhookData.task_id}`);

        // Mark task as deleted instead of actually deleting it
        await this.db.query(`
        UPDATE tasks 
        SET is_active = false, sync_status = 'synced', last_sync_at = NOW()
        WHERE clickup_id = ?
    `, [webhookData.task_id]);
    }

    async handleCommentWebhook(webhookData) {
        if (this.syncInProgress.has(`comment-${webhookData.comment_id}`)) {
            console.log('Sync already in progress for this comment, skipping webhook');
            return;
        }

        this.syncInProgress.add(`comment-${webhookData.comment_id}`);

        try {
            // Fetch comments for the task
            const comments = await this.clickupAPI.getComments(webhookData.task_id);

            // Find the specific comment
            const comment = comments.comments.find(c => c.id === webhookData.comment_id);
            ...
            if (comment) {
                ...
                if (task.length > 0) {
                    await this.upsertComment(comment, task[0].id);
                }
            }
        } catch (error) {
            console.error('Failed to process comment webhook:', error);
        } finally {
            this.syncInProgress.delete(`comment-${webhookData.comment_id}`);
        }
    }

    async pushComment(operation) {
        ...
        // Get task's ClickUp ID
        const task = await this.db.query(
            'SELECT clickup_id FROM tasks WHERE id = ?',
            [commentData.task_id]
        );
        ...
        if (operation.operation_type === 'create') {
            // Create new comment
            const clickupCommentData = {
                comment_text: commentData.comment_text
            };

            const result = await this.clickupAPI.createComment(task[0].clickup_id, clickupCommentData);

            // Update local comment with ClickUp ID
            await this.db.query(
                'UPDATE comments SET clickup_id = ?, sync_status = "synced", last_sync_at = NOW() WHERE id = ?',
                [result.id, commentData.id]
            );
        }
    }

    async pushTimeEntry(operation) {
        ...
        const timeEntryData = timeEntry[0];

        // Get team's ClickUp ID
        const team = await this.db.query(
            'SELECT clickup_id FROM teams WHERE id = ?',
            [timeEntryData.team_id]
        );
        ...
        if (operation.operation_type === 'create') {
            const clickupTimeData = {
                description: timeEntryData.description,
                time: timeEntryData.time,
                start: timeEntryData.clickup_created_at.getTime(),
            ...
            if (timeEntryData.task_id) {
                ...
                if (task.length > 0) {
                    clickupTimeData.task = task[0].clickup_id;
                }
            }

            const result = await this.clickupAPI.createTimeEntry(team[0].clickup_id, clickupTimeData);

            // Update local time entry with ClickUp ID
            await this.db.query(
                'UPDATE time_entries SET clickup_id = ?, sync_status = "synced", last_sync_at = NOW() WHERE id = ?',
                [result.data.id, timeEntryData.id]
            );
        }
    }

    async applyClickUpData(entityType, entityId, clickupData) {
        // This method applies ClickUp data when resolving conflicts with "clickup_wins"
        const tableName = this.getTableName(entityType);

        switch (entityType) {
            case 'task':
                await this.applyClickUpTaskData(entityId, clickupData);
                break;
            case 'comment':
                await this.applyClickUpCommentData(entityId, clickupData);
                break;
            default:
                console.log(`ApplyClickUpData not implemented for ${entityType}`);
        }
    }

    async applyClickUpTaskData(taskId, clickupTaskData) {
        await this.db.query(`
        UPDATE tasks 
        SET name = ?, description = ?, due_date = ?, start_date = ?,
            time_estimate = ?, sync_status = 'synced', last_sync_at = NOW()
        WHERE id = ?
    `, [
            clickupTaskData.name,
            clickupTaskData.description || '',
            clickupTaskData.due_date ? new Date(parseInt(clickupTaskData.due_date)) : null,
            clickupTaskData.start_date ? new Date(parseInt(clickupTaskData.start_date)) : null,
            clickupTaskData.time_estimate || null,
            taskId
        ]);
    }

    async applyClickUpCommentData(commentId, clickupCommentData) {
        await this.db.query(`
        UPDATE comments 
        SET comment_text = ?, sync_status = 'synced', last_sync_at = NOW()
        WHERE id = ?
    `, [
            clickupCommentData.comment_text || '',
            commentId
        ]);
    }
    ...
    startPeriodicSync() {
        ...
        console.log('Periodic sync scheduled (every 1 minutes)');
    }
